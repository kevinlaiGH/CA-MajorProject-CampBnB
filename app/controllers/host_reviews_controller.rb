...
class HostReviewsController < ApplicationController
  ...
  def create
    # Step 1: Check if the reservation exist (accommodation_id, guest_id, host_id)

    # Step 2: Check if the current host already reviewed the guest in this reservation.

    @reservation = Reservation.where(
                    id: host_review_params[:reservation_id],
                    accommodation_id: host_review_params[:accommodation_id],
...
#go back to the curretn page
  ...
  private
    def host_review_params
      params.require(:host_review).permit(:comment, :star, :accommodation_id, :reservation_id, :guest_id)
    end
end
